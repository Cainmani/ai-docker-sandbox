...
function Parse-WSLConfig {
    ...
    if ($content -match '(?mi)^memory\s*=\s*(.+)$') {
        $result.Memory = $Matches[1].Trim()
    }
    if ($content -match '(?mi)^swap\s*=\s*(.+)$') {
...
$script:lblLightDesc = New-Label '                 For 8GB systems. Basic Docker operations, single containers.' 40 254 800 20 9 $false $false
...
$script:lblStandardDesc = New-Label '                 For 16GB systems. Multiple containers, moderate builds.' 40 309 800 20 9 $false $false
...
$script:lblHeavyDesc = New-Label '                 For 24GB+ systems. Complex builds, heavy workloads.' 40 364 800 20 9 $false $false
