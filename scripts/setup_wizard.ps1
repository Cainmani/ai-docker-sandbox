...
function Get-SystemMemoryGB {
    try {
        ...
        $totalMemoryGB = [Math]::Round($totalMemoryKB / 1024 / 1024, 1)
...
$state = [ordered]@{
    ...
    ParentPath = ''
    WorkspacePath = ''
    WSLProfile = ''          # 'light', 'standard', 'heavy', or 'skip'
...
$p4_wsl.Controls.Add((New-Label 'Detected System:' 20 80 880 24 10 $true $false))
...
$p4_wsl.Controls.Add((New-Label 'Select a profile:' 20 200 880 24 10 $true $false))
...
$lblLightDesc = New-Label '                 For 8GB systems. Basic Docker operations, single containers.' 40 254 800 20 9 $false $false
$lblLightDesc.ForeColor = $script:MatrixAccent
$p4_wsl.Controls.Add($lblLightDesc)
...
$lblStandardDesc = New-Label '                 For 16GB systems. Multiple containers, moderate builds.' 40 309 800 20 9 $false $false
$lblStandardDesc.ForeColor = $script:MatrixAccent
$p4_wsl.Controls.Add($lblStandardDesc)
...
$lblHeavyDesc = New-Label '                 For 24GB+ systems. Complex builds, heavy workloads.' 40 364 800 20 9 $false $false
$lblHeavyDesc.ForeColor = $script:MatrixAccent
$p4_wsl.Controls.Add($lblHeavyDesc)
...
# Information text
$p4_wsl.Controls.Add((New-Label '' 20 440 880 24 10 $false $true))
$p4_wsl.Controls.Add((New-Label 'This creates a .wslconfig file to prevent WSL from consuming all your RAM.' 20 460 880 24 9 $false $true))
$p4_wsl.Controls.Add((New-Label 'Changes will apply after Docker Desktop restarts (happens automatically during setup).' 20 480 880 24 9 $false $true))
...
$btnNext.Add_Click({
    ...
    switch ($script:current) {
        ...
        3 {
            ...
            # Check if .wslconfig already exists - if so, skip WSL configuration page
            $wslconfigPath = "$env:USERPROFILE\.wslconfig"
            if (Test-Path $wslconfigPath) {
                Write-Host "[INFO] .wslconfig already exists at $wslconfigPath - skipping WSL configuration" -ForegroundColor Cyan
